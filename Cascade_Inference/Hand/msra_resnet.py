                    ...
                    if isinstance(m, nn.Conv2d):
                        ...
                        if m.weight.shape[0] == self.heads[head]:
                            if 'hm' in head:
                                ...
                                nn.init.normal_(m.weight, std=0.001)
                                nn.init.constant_(m.bias, 0)
            if str(num_layers) in "resnet18":
                model_path = "./finetune_model/resnet18-5c106cde.pth"
...
if __name__ == "__main__":
    ...
    model = get_pose_net(num_layers=18, heads={'hm': 80, 'wh': 2, 'reg': 2},  head_conv=64, pretrained=False)
    output = model(img)
    output = output[0]
