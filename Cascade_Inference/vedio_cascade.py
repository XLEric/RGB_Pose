...
if __name__ == '__main__':
    ...
    video_capture = cv2.VideoCapture('./vedio/6.mp4')
    idx = 0

    while True:
        ret, im_ = video_capture.read()
        if ret == False:
            break
        if ret:
            idx += 1
            ...
            if detections_persons is not None:
                ...
                for *xyxy, conf, cls_conf, cls in detections_persons:
                    ...
                    label_person_ = '{}:{:.2f}'.format(label,cls_conf)
                    plot_one_box(xyxy, im0, label=label_person_, color=colors[int(cls)])
